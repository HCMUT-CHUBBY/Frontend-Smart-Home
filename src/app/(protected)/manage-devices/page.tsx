...
export default function ManageDevicesPage() {
  ...
  // Xử lý submit từ AddEditDeviceModal
  const handleModalSubmit = async (deviceData: DeviceDTO) => {
    const isEditing = !!editingDevice;
    const apiCall = isEditing
      ? apiClient.put(`/devices/${editingDevice!.id}`, deviceData)
      : apiClient.post('/devices', deviceData);

    const successMessage = isEditing ? 'Device updated successfully!' : 'Device added successfully!';
    const failureMessage = isEditing ? 'Failed to update device' : 'Failed to add device';

    try {
      const response = await apiCall;
      toast.success(response.data?.message || successMessage);
      handleModalClose();
      await fetchDevices(); // Fetch lại để cập nhật danh sách
    } catch (err: unknown) {
      console.error(`${failureMessage}:`, err);
      const errorMessage = (err as { response?: { data?: { message?: string } } })?.response?.data?.message
        || (err instanceof Error ? err.message : "Unknown error occurred.");
      toast.error(`${failureMessage}: ${errorMessage}`);
      // Có thể muốn ném lỗi lại để modal xử lý
      // throw err;
    }
  };
  ...
  return (
    ...
    <div className="p-4 md:p-6 bg-gradient-to-br from-gray-50 to-blue-50 min-h-screen">
      ...
      <DeleteConfirmationModal
        ...
        // Lấy tên feed hoặc ID để hiển thị xác nhận
