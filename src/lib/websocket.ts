...
export const disconnectWebSocket = () => {
  ...
  if (stompClient && stompClient.connected && stompClient.active) {
      ...
      try {
       ...
       console.warn("STOMP client is active but not connected. Attempting deactivate anyway...");
        try {
            stompClient.deactivate();
            console.log("STOMP client deactivate called (was active but not connected).");
        } catch(e) {
            console.error("Error during stompClient.deactivate() (active but not connected):", e);
        } finally {
            stompClient = null;
        }
  } else {
      console.log("STOMP client not active or already null. No need to disconnect.");
  }
};
/**
